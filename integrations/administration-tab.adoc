---
sidebar: sidebar 
permalink: integrations/administration-tab.html 
keywords: performance service levels, keystone-bluexp, modify subscription, edit capacity, manage subscription, NetApp Console, Console 
summary: 查看或取消Keystone訂閱服務請求。 
---
= 查看Keystone訂閱的服務請求
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
:linkattrs: 
:imagesdir: ../media/


[role="lead"]
您可以透過NetApp控制台檢視和追蹤修改Keystone訂閱的服務要求，提供快速摘要並監控進度。

若要查看進度或取消請求，請依照下列步驟操作：

.步驟
. 從控制台左側導覽功能表中，選擇「儲存」> “Keystone” >「管理」>「報告」。
+
您會看到所有服務請求的列表，包括請求編號、請求類型和目前狀態。

+

NOTE: Keystone訂閱用戶目前無法使用「代理管理」和「報告」標籤。

. 您可以點選image:down-arrow.png["向下箭頭圖標"]*提交日期*列旁的圖示可展開請求編號並查看詳細資料。您會看到提出請求的效能服務等級。
+
image:console-service-request-list.png["螢幕截圖顯示了修改請求的效能服務級別"]

. 選擇請求編號可查看詳細信息，包括所請求訂閱的修改詳細信息和當前進度狀態。
+
image:bxp-service-progress.png["顯示當前進度狀態的螢幕截圖"]


