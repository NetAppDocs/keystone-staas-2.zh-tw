...
+
您可以查看 Keystone 訂閱中的所有警告監控器列表，包括監控器名稱、觸發通知的監控條件、嚴重等級、訂閱編號、效能服務等級和狀態。您可以查看 *Created by* 欄，以了解警告監控器是系統產生的還是使用者定義的。

+
Keystone 會自動為您的所有訂閱建立系統產生的監控器。這些監控器無法刪除。編輯功能僅限於修改容量使用情況監控器的訂閱數量和效能服務層級，以及訂閱到期監控器的訂閱數量。

+
** *訂閱到期監控*：在訂閱到期日前的五個閾值（180 天、90 天、60 天、30 天和 7 天）觸發警報。隨著到期日臨近，嚴重性從資訊性升級至關鍵。
** *Capacity usage monitor*：警報是基於您訂閱的承諾突發容量百分比，而非固定值。例如，如果您的承諾突發容量為 40%，則當容量使用率達到承諾容量的 140% 時，將觸發關鍵警報。
+
...
NOTE: 您必須被指派到 * Keystone管理員 * 角色才能建立和管理警報監控器。欲了解更多信息，請參閱 link:https://docs.netapp.com/us-en/console-setup-admin/reference-iam-keystone-roles.html["NetApp Console中的Keystone存取角色"^]。
