...
+
image:console-subscription.png["查看訂閱標籤詳情"]
...
** *連結的訂閱*：如果您有任何連結至主要訂閱的次要訂閱，此欄會列出主要訂閱的連結訂閱編號。此欄預設為隱藏，僅在有連結的訂閱時才會顯示。
** *Keystone 版本*：訂閱所使用的 Keystone 服務版本。不同訂閱版本的效能服務等級費率方案規則可能有所不同。要了解有關 v1 版本的更多資訊，請參閱 https://docs.netapp.com/us-en/keystone/index.html["Keystone訂閱服務 | 版本 1 文檔"^]。此欄預設為隱藏，可以使用欄選擇器 image:column-selector.png["列選擇器圖示"] 圖示啟用。
...
** *最高容量*：訂閱內消耗的最大容量。
** *訂閱到期天數*：在到期日旁顯示一個圖示，指示訂閱距離到期還有多久，以及是否會自動續訂。此狀態為唯讀，反映您的合約條款。若要審查合約詳情，請參閱 link:../integrations/subscription-timeline.html["查看Keystone訂閱的時間表"]。
+
自動續約關閉時，圖示顏色會指示到期時間臨近程度：

+
*** *綠色*：距離到期日還有超過 120 天。
*** *橘色*：距離到期日還有 91 至 120 天。
*** *紅*：距離到期日不到 90 天。為避免服務中斷，請向 NetApp Keystone 支援提交續約申請。
*** *灰色*：訂閱已過期。
+
對於已開啟自動續訂的訂閱，會顯示自動續訂圖示，表示訂閱將在目前期限結束時自動續訂。
